...
pnodo delentfixup (pnodo tree, pnodo nil, pelen lista, char* str);
...
int main()
	{char st1[35], st2[35], st3[35], st4[35], ing[L];
	 char strend[]= "end\n";
	 int i,j, cont=0, contatore=0, contafile=0, temp1, temp2;
	 pelen elenco, rimand;  
	 pnodo rif, nullo, rinv;
	 FILE *fp;
	 fp=fopen("test2-1.txt", "r");
							 ...
							 while (rimand!=NULL)  //per tutto l'elenco degli alberi delle relazioni
								{if (rimand->radice!=nullo)   //se questo albero non è vuoto
									...
									if (rimand->radice!=nullo) 
											{rimand->radice=delentfixup(rimand->radice, nullo, rimand, st2);} //in seguito fixo gli elenchi dei mittenti
									}
							...
							{elenco->next=nuovarelazione(elenco->next, nullo, st4);
							 ...
							 if (rif==nullo) //se il destinatario non è presente
									{rimand->radice=treeinsert(rimand->radice, nullo, st3); //aggiungo la entity all'albero della relazione
									 ...
									 rif->nm++;
									 //branca per fixare la report
									 if (rif->nm==rimand->max)
											{rimand->stamp++;}   //se è uguale al massimo presente aggiungo uno al numero di massimi
									 if (rif->nm > rimand->max && rimand->stamp> 1 )  // sennò 
											{rimand->stamp2=rimand->stamp;
											 rimand->max2=rimand->max;   //copio i dati del 2° massimo
											 rimand->stamp=1;
											 rimand->max=rif->nm;   //se è maggiore imposto il nuovo massimo e riduco a 1 il numero dei massimi
											}
									 else if (rif->nm >rimand->max && rimand->stamp==1)
											{rimand->max=rif->nm;}
												
									 else if (rif->nm > rimand->max2)
											{rimand->max2=rif->nm;
											 rimand->stamp2=1;
											}
									 else if (rif->nm==rimand->max2)
									...
									{if (treesearch(rif->firstmitt, nullo, st2)==nullo)  //se la entity c'è aggiungo solamente il mittente, se serve
											{rif->firstmitt=treeinsert(rif->firstmitt, nullo, st2);
											 ...
											 if (rif->nm==rimand->max)
													{rimand->stamp++;}   //se è uguale al massimo presente aggiungo uno al numero di massimi
											 if (rif->nm > rimand->max && rimand->stamp> 1 )
													{rimand->stamp2=rimand->stamp;
													 rimand->max2=rimand->max;   //copio i dati del 2° massimo
													 rimand->stamp=1;
												     rimand->max=rif->nm;   //se è maggiore imposto il nuovo massimo e riduco a 1 il numero dei massimi
					...
					{if (treesearch(elenco->radice, nullo, st2)==nullo || treesearch(elenco->radice, nullo, st3)==nullo)
					 ...
					 if (rinv==nullo)
							...
							{rif->firstmitt=delent(rif->firstmitt, rinv, nullo);
						     if (rif->nm==rimand->max)	                   //se ho toccato il mittente con il massimo
									{if (rimand->stamp==1)  //se aveva una sola occorrenza controllo il primo max, se minore del secondo li inverto
											{rif->nm--;
										     if (rif->nm==rimand->max2)
												{rimand->max=rimand->max2;
												 rimand->stamp=rimand->stamp2;
												 rimand->stamp++;
												 rimand->max2--;
												 rimand->stamp2=1;
												}
											...
											{rif->nm--;
											 rimand->max--;
											}					//se c'era un altro con tanti mittenti decremento solo nm
									}
							 else 
									{rif->nm--;}
...
pnodo delentfixup (pnodo tree, pnodo nil, pelen lista, char* str)  //questa funzione elimina dagli elenchi dei mittenti una entity cancellata
	{pnodo punt;
	 pelen rimand;
	 rimand=lista;
	 ...
	 if (tree->nm==rimand->max)	                   //se ho toccato il mittente con il massimo
									{if (rimand->stamp==1)  //se aveva una sola occorrenza controllo il primo max, se minore del secondo li inverto
											{tree->nm--;
										     if (tree->nm==rimand->max2)
												{rimand->max=rimand->max2;
												 rimand->stamp=rimand->stamp2;
												 rimand->stamp++;
												 rimand->max2--;
	 ...
	 if (tree->left!=nil)
			{tree->left=delentfixup(tree->left, nil, rimand, str);}
	 if (tree->right!=nil)
			{tree->right=delentfixup(tree->right, nil, rimand, str);}   //propago nel sottoalbero sinistro o destro se questi sono diversi da nil
	 return tree;
	}	
